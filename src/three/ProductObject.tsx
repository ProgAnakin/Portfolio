...
import { PRODUCT_SCALE, shapeHeight } from './shapeMetrics';
...
export function ProductObject({ project, position }: ProductObjectProps) {
  ...
  useFrame((_, dt) => {
    ...
    const targetLift = state.dragging ? 0.3 : active ? 0.17 : 0;
    // Squashes as it leaves the shelf and stretches at the top of the lift —
    // volume is conserved, which is what makes it read as a toy and not a card.
    ...
    group.current.position.set(
      position[0] + dragX.current.value,
      position[1] + lift.current.value + dragY.current.value,
      position[2],
    );
    group.current.rotation.y = spin.current.value + dragX.current.value * 0.55;
    group.current.rotation.z = -dragX.current.value * 0.2;

    const s = PRODUCT_SCALE;
    group.current.scale.set(
      s * (2 - squash.current.value),
      s * squash.current.value,
      s * (2 - squash.current.value),
    );
  ...
  return (
    <group ref={group} position={position}>
      <Shape finish={project.finish} color={color} label={label} lit={!soldOut} />
